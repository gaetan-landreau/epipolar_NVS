...
from utils.utils_model import BilinearSamplingLayer, get_modified_decoder_layer
...
class ModelZhou16Attention(ModelInterface):
    ...
    def build_model(self):
        ...
        image_input = Input(
            shape=(current_image_size, current_image_size, 3), name="image_input"
        )
        ...
        while current_image_size > 4:
            x = Conv2D(
                16 * (2**i), kernel_size=(3, 3), strides=(2, 2), padding="same"
            )(x)
            ...
            if (
                ...
                if current_image_size < self.image_size:
                    pred_flow = Conv2DTranspose(
                        2, kernel_size=(3, 3), strides=(2, 2), padding="same"
                    )(d)

            # generate next decoder layer x^(l)_d from previous decoder layer x^(l+1)_d
            d = Conv2DTranspose(
                channel_size, kernel_size=(3, 3), strides=(2, 2), padding="same"
            )(d)
        ...
        # final flow
        pred_flow = Conv2DTranspose(
            2, kernel_size=(3, 3), strides=(1, 1), padding="same"
        )(d)
    ...
    def process_pose_info(self, data: DataLoader, pose_info):
        ...
        if data.name == "chair" or data.name == "car":
            ...
            transformation_azimuth = np.remainder(
                target_azimuth - source_azimuth + data.n_azimuth, data.n_azimuth
            )
            transformation_azimuth_onehot = np.eye(data.n_azimuth, dtype=np.float32)[
                transformation_azimuth
            ]
