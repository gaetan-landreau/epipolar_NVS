...
from utils.dense_wrap import dense_image_warpV2
...
class BilinearSamplingLayer(Layer):
    ...
    def call(self, tensors):
        ...
        return dense_image_warpV2(original_image, predicted_flow * self.image_size)
...
def index_to_sin_cos(
    array: np.ndarray, max_index, loop=True, min_theta=0, max_theta=2 * np.pi
):
...
def get_modified_decoder_layer(
    x_d0, x_e, current_attention_strategy, current_image_size, pred_flow=None
):
    ...
    # (4) Flow-based Hard Attention
        ...
        # print(f'Size of tensor x_d0: {x_d0.shape}')
        # print(f'Size of tensor x_e_rearranged: {x_e_rearranged.shape}')
