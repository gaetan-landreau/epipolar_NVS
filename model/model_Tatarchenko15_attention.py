...
class ModelTatarchenko15Attention(ModelInterface):
    ...
    def build_model(self):
        ...
        concatenated = Dense(hidden_layer_size, activation=activation)(concatenated)
        concatenated = Dense(hidden_layer_size, activation=activation)(
            concatenated
        )  # Only keep 3 dense layer since there is no more concatenation.
    ...
    def process_pose_info(self, data: DataLoader, pose_info):
        ...
        if data.name == "chair" or data.name == "car":
            ...
            target_elevation_cos_sin = index_to_sin_cos(
                target_elevation,
                data.n_elevation,
                loop=False,
            ...
            target_view = np.stack(
                (
                    np.ones(
                        batch_size,
                    ),
                    target_azimuth_cos_sin[0],
                    target_azimuth_cos_sin[1],
                    target_elevation_cos_sin[0],
                    target_elevation_cos_sin[1],
                ),
                axis=-1,
            )
...
if __name__ == "__main__":
    tModel = ModelTatarchenko15Attention(image_size=256)
    tModel.build_model()
